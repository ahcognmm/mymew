...
* **Name/action styling:** Only the dot's color was ever wire-marked; the surrounding `name: action` text used to land in the terminal's default color (the dot's own trailing reset clears it), reading as plain prose indistinguishable from the rest of the transcript. `runToolLoopDispatchCall` now wraps `name` in bold-cyan and `action` in dim SGR alongside the dot when it writes the marker, so a tool call is visually a tool call even before its dot resolves. This is additional coloring only — the ✓/✗/● glyph distinction (previous bullet) remains the monochrome/CVD-safe fallback.
...
### **3.9. Human-in-the-Loop Approval**

For a DevOps-facing agent, "reliable" means destructive actions never run unattended. The engine exposes a type-erased, blocking approval bridge — `hook.Approver` (`core/hook.zig`) — rather than a new orchestrator hook point, because the existing `preTool` veto mechanism (§3.7) already covers everything needed: a hook decides whether a call proceeds, and vetoing is not a self-healing failure.

* **The bridge.** `Approver` is `{ ptr: *anyopaque, requestFn: *const fn(ptr, question: []const u8) bool }`. `Engine.setApprover()` registers one before the first `step()`; it flows into `Ctx.approver` at both `Ctx` construction sites (turn-level and loop-level) alongside `style`. `request()` **blocks the orchestrator thread** until answered. No approver attached (e.g. the headless `-p` path) means `Ctx.approver == null` — a policy hook must fail closed (veto) in that case, never proceed by default.
* **The TUI side.** `core/agent.zig`'s `Agent.spawn()` registers itself as the approver (`requestApproval`): it parks the question in `io_bus.AgentState` (`approval_question`/`approval_answer`/`approval_pending`, guarded by the existing `mutex`/`cond`), pings the wakeup fd, and blocks on the condvar until the render thread calls `answerApproval()` — or until `requestCancel()` fires, which the wait treats as a deny so Esc always has a way out instead of dangling forever (§3.4's cancel convention extended to this new blocking point). The render loop (`main.zig`) polls `agent.approvalPending()` each frame, surfaces the question via `Tui.setApproval()` (bar's top-priority state, above busy/scrolled — err-colored `● allow? <command> · y run · n deny`), and routes bare `y`/`n` keypresses to `agent.answerApproval()` instead of the composer while active.
* **The policy hook.** `plugins/hooks/command_guard.zig` is a `preTool` hook scoped to `execute_command`: `classify()` matches the command against destructive phrases (`terraform destroy`, `git push -f`, `kubectl delete`, `DROP TABLE`, ...) and single leading words checked only in command position (`rm`, `dd`, `sudo`, `kill`, ...) so `echo rm` or a filename containing "dd" never false-positives. A match with no approver vetoes outright with a "do not retry" message; a match with an approver blocks on `request()` and vetoes or proceeds based on the answer — either way the veto text is written back as the tool result so the model sees exactly what happened and doesn't retry.
* **Ownership.** Same as every other hook: the veto message is `ctx.scratch`-owned; the approval question passed to `request()` only needs to live for the call (the TUI copies it to gpa before showing it, freed after `setApproval` dupes it internally).

### **3.10. Reliability Baseline for Tool Truthfulness**

A DevOps agent that hallucinates success is worse than one that refuses. `execute_command` (`plugins/tools/execute_command.zig`) was rewritten off `popen()` (which discards the real exit status) onto `fork`/`execve`/`waitpid`: every result is prefixed with `exit code: N` / `killed by signal N` / a timeout notice, never just raw output text the model would have to infer success from. A `timeout_seconds` arg (default 120s) is enforced by polling the child's stdout/stderr pipe against a deadline; on expiry (or 512KB output) the whole process group is killed (`setpgid` in the child + `kill(-pid, ...)`) so pipelines don't leak orphaned processes, and the result says so explicitly rather than returning partial output silently. `plugins/hooks/system_prompt.zig` reinforces this at the model level: a `preLlm` hook (same view-only, re-checked-every-iteration pattern as `todo_tracker`) pins `prompts/devops.md` — rules against assuming state, proceeding past a non-zero exit code, or inventing values — at position 0 of every outbound view via `@embedFile`, so the instruction survives `zig build` without touching `core/*`.
...
* **Main Chat View:** scrollable transcript viewport (top). Provider reasoning renders inside a dim, italic, cyan-tinted "thinking" span (`\x1b[2;3;36m…\x1b[0m` — cyan added alongside dim/italic so the span still reads on terminals where italic renders as plain text) with a matching cyan-guttered bar; escalation reports and turn errors render inside a red-guttered error span (`\x1b[2;31m…\x1b[0m`) — this gutter-block treatment replaced an earlier "Inspector Sidebar" idea: diagnostics stay in the one transcript, visually set off, rather than in a second panel. A third gutter, magenta-barred with a dark background wash, marks ` ```-fenced ` code blocks in the assistant's own reply; unlike the other two spans it's detected from the plain fence bytes at render time rather than a wire SGR marker, since fenced code is just markdown the model wrote, not something the provider/engine layer marks up. `` `Inline code` `` backticks within ordinary prose rows get a lighter bold-accent highlight the same way. Tool-call marker lines (§3.6) similarly color the name/action text, not just the status dot, so a pending or completed tool call is visually distinct from surrounding prose.
